    ...
    switch (type->kind) {
        case Type_Basic: {
            ...
            if (basic->identifier.kind == Identifier_Single) {
                ...
                for (size_t i = 0; i < basic->identifier.data.multi.count; i++) {
                    printf("%s", basic->identifier.data.multi.elements[i]);
                    if (i < basic->identifier.data.multi.count - 1) {
                        printf("::");
                    }
                }
            }
...
Basic_Type identifier_to_basic_type(Identifier identifier) {
    ...
    if (identifier.kind == Identifier_Single) {
        ...
        basic.identifier.data.multi = identifier.data.multi;
...
Type clone_macro_type(Type type, Array_String bindings, Array_Macro_Syntax_Data values) {
    ...
    switch (type.kind) {
        ...
        case Type_Struct: {
            Struct_Type* struct_in = &type.data.struct_;
            ...
            for (size_t i = 0; i < struct_in->items.count; i++) {
                ...
                array_declaration_pointer_append(&struct_out.items, declaration);
            }
...
Macro_Syntax_Data clone_macro_syntax_data(Macro_Syntax_Data data, Array_String bindings, Array_Macro_Syntax_Data values);
                ...
                *syntax_data = clone_macro_syntax_data(*run_macro_in->arguments.elements[i], bindings, values);
                ...
                if (syntax_data->kind.kind == Macro_Multiple_Expanded) {
                    for (size_t j = 0; j < syntax_data->data.multiple_expanded.count; j++) {
                        array_macro_syntax_data_append(&run_macro_out.arguments, syntax_data->data.multiple_expanded.elements[j]);
                    }
                } else {
                    array_macro_syntax_data_append(&run_macro_out.arguments, syntax_data);
                }
            }
                        ...
                        for (size_t i = 0; i < bindings.count; i++) {
                            if (strcmp(bindings.elements[i], identifier.data.single) == 0) {
                                if (strcmp(bindings.elements[i], "..") == 0) {
                                    ...
                                    Multi_Expression_Node multi = { .expressions = array_expression_node_new(2) };

                                    for (size_t j = i; j < values.count; j++) {
                                        array_expression_node_append(&multi.expressions, values.elements[j]->data.expression);
                                    }

                                    expression.kind = Expression_Multi;
                                    expression.data.multi = multi;
    ...
    switch (data.kind.kind) {
        ...
        case Macro_Multiple: {
            if (data.kind.data.multiple->kind == Macro_Expression) {
                Macro_Syntax_Data inner = *data.data.multiple;
                inner = clone_macro_syntax_data(inner, bindings, values);
                assert(inner.data.expression->kind == Expression_Multi);

                Array_Macro_Syntax_Data datas = array_macro_syntax_data_new(2);
                for (size_t i = 0; i < inner.data.expression->data.multi.expressions.count; i++) {
                    Macro_Syntax_Data* individual = malloc(sizeof(Macro_Syntax_Data));
                    individual->kind.kind = Macro_Expression;
                    individual->data.expression = inner.data.expression->data.multi.expressions.elements[i];

                    array_macro_syntax_data_append(&datas, individual);
                }
...
bool macro_syntax_kind_equal(Macro_Syntax_Kind wanted, Macro_Syntax_Kind given) {
    if (wanted.kind == given.kind) {
    ...
    switch (type->kind) {
        ...
        case Type_RunMacro: {
            ...
            for (size_t i = 0; i < run_macro->arguments.count; i++) {
                Macro_Syntax_Kind current_macro_argument = macro->arguments.elements[current_macro_argument_index];
                Macro_Syntax_Kind given_argument = run_macro->arguments.elements[i]->kind;
            ...
            if (!found && retrieve->kind == Retrieve_Assign_Identifier) {
                ...
                switch (resolved.kind) {
                    ...
                    case Resolved_Enum_Variant: {
                        ...
                        if (basic.identifier.data.multi.count > 2) {
                            basic.identifier.data.multi.count--;
                        } else {
                            basic.identifier.kind = Identifier_Single;
                            basic.identifier.data.single = basic.identifier.data.multi.elements[0];
                        }
